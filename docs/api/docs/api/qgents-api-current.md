...
repositoryMappings
array
当前 Task Workspace 已挂载的仓库映射，可为空数组；按 workspacePath 稳定排序
repositoryMappings[].workspacePath
string
Workspace 内一级相对目录，例如 repo-2；不是宿主机绝对路径
repositoryMappings[].repositoryId
UUID
项目仓库绑定 ID（project_repositories.id），不是 GitHub 数字仓库 ID
repositoryMappings[].name
string/null
仓库短名称；元数据缺失时为 null
repositoryMappings[].fullName
string/null
Git 提供方 owner/name；元数据缺失时为 null
repositoryMappings[].provider
string/null
Git 提供方，例如 GITHUB
repositoryMappings[].baseRef
string/null
当前 Task 在该仓库固定的基线分支
repositoryMappings[].sourceBranch
string/null
当前 Task 在该仓库的源分支
currentRepositoryPaths
string[]
当前 TaskStep、Diff 批次、Dry Run 或 MR 实际涉及的 Workspace 路径，可为空数组
    ...
    "repositoryMappings": [
      {
        ...
        "repositoryId": "project-repository-uuid",
        "name": "testtesttest",
        "fullName": "Choco-emmm/testtesttest",
        "provider": "GITHUB",
        "baseRef": "main",
        "sourceBranch": "feat/task-uuid"
      }
    ],
    "currentRepositoryPaths": ["repo-2"]
  }
}

语义约束：

- repositoryMappings 表示当前 Task Workspace 的完整仓库映射；
- currentRepositoryPaths 表示当前卡片所对应的实际步骤或交付事实涉及的仓库，不得默认取第一个仓库；
- Planner 尚未创建 Workspace 或当前步骤没有仓库范围时返回空数组，不代表任务失败；
- 仓库被软解绑、归档或远程元数据缺失时，仍尽量保留 repositoryId 和 workspacePath，名称字段可为 null；
- 客户端应使用 currentRepositoryPaths 查找映射并优先展示 fullName，再回退到 name 或 repositoryId；
- 客户端不得根据 Planner 文本、repo-1/repo-2 路径或项目默认仓库自行猜测当前仓库；
- 实时消息、历史消息、SSE/WebSocket 载荷使用同一字段语义；
- 字段不得包含宿主机绝对路径、Token、凭据、命令输出或完整文件列表。

40. Skill 与 Agent 管理员直发免审批（2026-08-20）

管理员发布自己创建的 Skill 或 Agent 时，不需要再把自己作为审核人重复交付。管理员身份由服务端根据真实项目/团队成员关系判断，客户端不得提交或伪造 role、admin 等权限字段。

40.1 Skill

- PRIVATE Skill：任何项目成员创建即为 PUBLISHED，仅创建者可见和使用，不进入交付中心，也不能通过 submit-review 转为项目共享。
- PROJECT_SHARED Skill：Project Admin 创建时直接为 PUBLISHED，写入创建者为 reviewer 并发布 skill.published 事件；不会进入 PENDING_REVIEW，不需要再次调用 approve。
- 普通项目成员创建 PROJECT_SHARED Skill 时为 DRAFT，之后仍按 submit-review -> Project Admin approve/reject 流程处理。
  
创建接口仍为：

POST /api/v1/projects/{projectId}/skills

请求体中的 visibility 为 PRIVATE 或 PROJECT_SHARED。响应中的 status 是前端判断结果的唯一依据：管理员共享 Skill 直接返回 PUBLISHED，普通成员共享 Skill 返回 DRAFT。

40.2 Agent

Agent 属于 Team 级资源。普通创建者发布自己的 Agent 时进入审核；Team Owner 发布自己创建的 Agent 时直接发布：

普通创建者：PRIVATE + ACTIVE --publish--> PENDING + ACTIVE
Team Owner 自建：PRIVATE + ACTIVE --publish--> TEAM + ACTIVE
普通创建者的 PENDING --approve（Team Owner）--> TEAM + ACTIVE
TEAM + ACTIVE --archive--> ARCHIVED

发布接口：

POST /api/v1/teams/{teamId}/agents/{agentId}/publish

- 发布者必须是 Agent 创建者，且 Agent 当前为 PRIVATE + ACTIVE。
- 普通创建者响应为 visibility=PENDING，等待 Team Owner 调用 approve。
- Team Owner 发布自己创建的 Agent 响应为 visibility=TEAM，并写入 reviewedBy、reviewedAt；不会生成待自己审批的项目。
- approve、reject 仅适用于普通成员提交的 PENDING + ACTIVE Agent；对 Team Owner 直发的 TEAM Agent 调用返回 409 AGENT_STATE_CONFLICT。
- TEAM Agent 不得通过 unpublish 退回 PRIVATE，只能归档；unpublish 返回 409 AGENT_UNPUBLISH_DISALLOWED。
  
40.3 交付中心与前端行为

- GET /api/v1/projects/{projectId}/delivery-items?type=SKILL 和 type=AGENT 只展示进入共享流程的资源。管理员直发的 Skill/Agent 直接以 displayStatus=ACCEPTED 返回，不出现 PENDING_REVIEW 待审核项。
- Skill 的 canApprove/canReject 仅在 PENDING_REVIEW 时为 true；Agent 的 canApprove/canReject 仅在 PENDING 且当前用户为 Team Owner 时为 true。
- 前端发布成功后必须使用响应状态刷新 TanStack Query 缓存：Skill status=PUBLISHED 显示“已发布”，Agent visibility=TEAM 显示“已发布为团队可用”；不得根据当前登录用户在前端自行推断管理员权限或伪造状态。
- 管理员直发不会产生额外的交付中心批准操作；普通成员的审核入口和原有 approve/reject 接口保持不变。

41. 手动发起任务的群聊提示

手动发起任务（POST /api/v1/projects/{projectId}/tasks，非 @Agent/消息触发）后，服务端以编排助手 Agent 身份在需求群插入一句 TEXT 文本消息，提示发起者任务已开始。不新增群聊消息类型，复用既有 TEXT 类型与 mentions 结构。
...
- 发送者为团队编排助手 Agent（senderType=AGENT，名称"编排助手"），团队查不到编排助手时跳过插入并记录日志，不阻断任务创建。
...
42. 任务启动失败错误码修复：GIT_BRANCH_NOT_FOUND（2026-08-24）
...
true
HTTP 语义
启动失败落 Task FAILED，TaskStatusReason.code = STARTUP_FAILED
...
44. 项目内新建 GitHub 仓库并绑定（2026-08-20）

44.1 适用场景

创建项目时，客户端仍使用项目创建接口中的 repositoryIds 与 newRepository 二选一：

- repositoryIds：绑定团队已授权的一个或多个已有仓库；
- newRepository：自动创建一个带初始提交的 GitHub 仓库并绑定。
  
项目创建完成后，如用户需要为已有项目准备一个全新的仓库，可使用本接口。该入口位于项目代码仓库页面，但新建的 GitHub 仓库仍是团队 Installation 下的仓库资源，不是项目私有资源；创建完成后可以通过普通绑定接口复用到其他项目。

一个项目可以绑定多个仓库，同一个 GitHub 仓库也可以绑定多个项目；唯一限制是同一项目不能重复绑定同一仓库。

44.2 创建并绑定

仅 Team Owner 可调用。请求必须携带 Idempotency-Key，由服务端幂等过滤器保证网络重试不会重复执行同一成功请求。
...
string
是
GitHub 仓库名，最长 100 字符；服务端按 GitHub 建仓规则校验
description
string
否
仓库描述，最长 500 字符
...
UUID
否
建仓使用的 Qgents Installation；团队只有一个 ACTIVE Installation 时可省略
displayName
string
否
仓库在当前项目内的显示名称，缺省取仓库名

服务端执行：

1. 校验项目存在；
2. 校验当前用户是该项目所属团队的 canonical TEAM_OWNER；
3. 校验 Installation 属于该团队、状态为 ACTIVE 且有有效 provider installation id；
4. 在数据库事务外调用 GitHub 创建仓库，并固定使用 auto_init=true；
5. 要求 GitHub 返回真实非空 defaultBranch；
6. 在本地事务内写入 github_repositories 镜像和 project_repositories 绑定；
7. 本地落库失败时尽力删除本次创建的远端仓库；
8. 返回完整的 ProjectRepository DTO。
...

{
  "data": {
    ...
    "installationId": "installation-local-uuid",
    "providerRepositoryId": 987654321,
    "fullName": "qgents/business-platform-backend",
    "githubUrl": "https://github.com/qgents/business-platform-backend",
    "defaultBranch": "main",
    "displayName": "后端仓库",
    "authorizationStatus": "AUTHORIZED",
    "metadataSyncedAt": "2026-08-20T08:00:00Z",
    "boundAt": "2026-08-20T08:00:00Z"
  },
  "requestId": "req_xxx"
}
...
- GET /teams/{teamId}/integrations/github/repositories 可以返回 defaultBranch=null 的空仓库；
- 普通绑定 POST /projects/{projectId}/repositories 对空仓库仍返回 409 GITHUB_REPOSITORY_METADATA_INCOMPLETE；
- 前端不得把空 defaultBranch 回退为 main，也不得让空仓库进入可提交的普通绑定流程；
- 远程分支创建接口仍然是“从已有分支创建新分支”，不负责初始化空仓库；
- 本接口创建的仓库必须由 GitHub 生成真实初始提交和默认分支，不产生可供任务使用的空绑定。
...
403
GITHUB_REPOSITORY_ACCESS_DENIED
当前用户不是项目所属团队的 Team Owner
404
GITHUB_RESOURCE_NOT_FOUND
项目不存在
422
GITHUB_INSTALLATION_NOT_ACTIVE
指定 Installation 不属于该团队、已不可用或缺少 provider id
422
GITHUB_INSTALLATION_REQUIRED
团队存在多个可用 Installation 但请求未指定
409
GITHUB_REPOSITORY_CREATE_CONFLICT
GitHub 仓库名冲突或不符合 GitHub 建仓约束
502
GITHUB_REPOSITORY_METADATA_INCOMPLETE
GitHub 创建成功但未返回可用默认分支；服务端会尝试补偿删除远端仓库
502
GITHUB_API_UNAVAILABLE
GitHub API 暂时不可用
...
- 创建项目时自动建仓：仍使用 POST /teams/{teamId}/projects 的 newRepository；
- 项目内绑定已有仓库：仍使用 POST /projects/{projectId}/repositories；
- 项目内新建并绑定仓库：使用本节新增接口；
- 创建远程分支：仍使用 POST /projects/{projectId}/repositories/{projectRepositoryId}/branches；
- 修改 Qgents 项目默认基准分支：仍使用 PATCH /projects/{projectId}/repositories/{projectRepositoryId}，不会修改 GitHub 全局默认分支。
  
44.6 创建项目时仓库来源互斥

POST /api/v1/teams/{teamId}/projects 中，repositoryIds 与 newRepository 是二选一字段：

- 传入 repositoryIds 时，只绑定已有的团队授权仓库；
- 传入 newRepository 时，只创建并绑定一个新的 GitHub 仓库；
- 两者均不传时，项目可以先创建，之后再通过项目仓库接口绑定或新建；
- 两者同时传入时，服务端拒绝请求，不会静默忽略任一字段，也不会创建项目或远端仓库。
  
错误响应：

{
  "error": {
    "code": "PROJECT_REPOSITORY_SOURCE_CONFLICT",
    "message": "repositoryIds 与 newRepository 不能同时传入"
  },
  "requestId": "req_xxx"
}
...
本轮项目仓库能力的接口补充统一追加在本节（第 44 节）末尾，前文既有接口章节保持不变。客户端实现项目级新建仓库、空仓库限制及项目创建时仓库来源互斥时，以第 44 节的接口、错误码和字段约束为准。

45. 任务失败原因展示统一

任务详情接口与任务诊断接口的任务级失败原因改为同一套组装逻辑（TaskStatusReasonFactory），消除同一页面两条链路返回矛盾失败信息的现象；同时修正白名单过滤导致真实原因丢失、质量循环耗尽失败码误导等问题。
...
code 恒为 STARTUP_FAILED，summary 用通用脱敏文案
与诊断链路一致：按失败阶段区分 code，summary 优先持久化 failureReason（脱敏）
任务诊断
GET /api/v1/projects/{projectId}/tasks/{taskId}/diagnostics
独立组装 taskFailureReason
复用同一工厂，历史数据回退失败 run 原因

code 语义：

场景
code
任务 FAILED 且存在 FAILED TaskRun
EXECUTION_FAILED
任务 FAILED 且无 TaskRun（启动即失败）
STARTUP_FAILED
任务 DELIVERY_FAILED 或失败码属交付阶段（TASK_FINALIZATION* / FINAL_* / 含 DIFF/DELIVERY/MR_）
DELIVERY_FAILED

45.2 summary 组装规则（白名单去副作用）
...
- 统一失败横幅：任务详情页仅保留 TaskFailureDiagnostic（读诊断接口）作为失败展示；启动失败横幅（TaskStartupFailureAlert）仅在没有失败 TaskRun 时出现（showStartupFailure={!hasFailedRun}），避免两横幅并存。
...
46. 统一创建 MR 自动预检补充契约（2026-08-20，尾部优先）

本节是对前文 MR 创建、预检、幂等和分支锁定描述的最终补充。若本节与前文存在差异，
以本节为准。该契约对应后端 fix/diff-review-latest-only 分支已合入 develop 的实现。

46.1 业务语义

POST /merge-requests/preflight 和兼容的 POST /merge-requests 都是“申请 MR 预检”，
不是立即创建 GitHub MR：

DIFF_FIRST：用户确认 Diff 并完成 commit/push
  -> 用户点击「创建 MR」
  -> 自动 Dry Run（加载 targetBranch 绑定的 Testset）
  -> 等待独立成员 CQ+1
  -> 后端自动、幂等创建真实 MR

MR_FIRST：用户确认交付并完成 commit/push
  -> 后端自动申请 Dry Run
  -> 等待独立成员 CQ+1
  -> 后端自动、幂等创建真实 MR

真实 MR 只能由 CQ+1 通过后的后端内部流程创建，客户端不能绕过 Dry Run 或 CQ+1。
...
sourceBranch、targetBranch、headCommit、targetCommit 和 Testset 均由后端从
Task Workspace、项目仓库绑定和 Git Store 读取。客户端不能覆盖这些字段。

兼容入口：

POST /api/v1/projects/{projectId}/merge-requests

该入口仍返回 202，但只转发到预检申请。旧请求中的 targetBranch 和 title 不参与
预检或 MR 创建决策，不表示真实 MR 已创建。

46.3 分支上下文幂等

预检属于仓库分支上下文，而不是单个 Task 独占资源。后端使用以下规范化字符串计算
SHA-256，并将 64 位十六进制结果持久化为 context_hash：

repositoryId|sourceBranch|targetBranch|headCommit|targetCommit

数据库唯一约束为：

UNIQUE KEY uk_mr_preflight_context_hash (context_hash)

原始分支名和 SHA 仍完整保存在 source_branch、target_branch、head_commit、
target_commit 中用于展示和校验；不再对这些长字符串建立联合唯一索引。

- 相同分支上下文重复申请返回同一预检记录，不会重复创建 Dry Run。
- 相同 Idempotency-Key 重复提交返回同一预检记录。
- source branch 或 target branch 的提交 SHA 变化时，必须生成新上下文，旧 Dry Run/CQ+1 失效。
- context_hash 不是客户端字段，客户端不得自行传入或替换。
  
46.4 多任务共用分支

多个已交付 DIFF_FIRST Task 可以连续提交到同一个 sourceBranch。用户从任一任务申请
预检时，后端以当前分支相对目标分支的完整累计差异为准，并在响应中返回：

- coveredTaskIds：本次分支级预检覆盖的全部已交付 Task；
- coveredDiffIds：这些 Task 在该仓库上已推送的 Diff；
- sourceBranch、targetBranch、headCommit、targetCommit：本次固定上下文。
  
因此不能把一个中间 Task 的单独 Diff 当作完整 MR 内容。预检开始后分支进入锁定状态，
禁止继续 push 改变 head；真实 MR 创建后继续向该分支开发也会被拒绝。

46.5 响应状态和分支锁定

MergeRequestPreflightResponse.status 以服务端持久化 Dry Run/CQ/MR 事实实时推导，
常见状态如下：

状态
含义
REQUESTED
预检申请已落库，尚未绑定 Dry Run
DRY_RUN_QUEUED
Dry Run 已受理
DRY_RUN_RUNNING
Dry Run 执行中
WAITING_CQ
Dry Run 通过，等待独立 CQ+1
CQ_REJECTED
CQ 被拒绝，可重新预检
CREATING_MR
CQ+1 已通过，后端正在创建真实 MR
MR_CREATED
真实 MR 已创建并回填本地镜像
FAILED
Dry Run 或预检流程失败，可按错误码判断是否重试

branchLockStatus 可能为：
...
src/main/resources/db/migration/V20260825_01__mr_preflight_request.sql

该迁移创建 mr_preflight_requests、mr_preflight_tasks，并使用 context_hash 唯一索引。
如果曾经执行过旧的超长联合索引 SQL，且建表失败，必须确认不存在半成品表后重新执行修正后的迁移。
禁止继续执行包含以下索引的旧 SQL：

UNIQUE KEY uk_mr_preflight_context (
  project_repository_id, source_branch, target_branch, head_commit, target_commit
)

46.7 前端状态恢复

前端收到 preflight.updated、dry-run.updated 或 merge-request.updated 后，必须重新查询
预检详情，不得根据按钮响应直接假设 MR 已创建。页面刷新、SSE 断线或服务重启后，使用：
...
恢复 status、dryRunId、coveredTaskIds、coveredDiffIds、branchLockStatus 和真实
mergeRequest 信息。
...
当前 Workspace 最新 Diff，等待用户确认
ACCEPTED
已确认并进入交付流程
REJECTED
用户拒绝该 Diff
SUPERSEDED
已被同一 Workspace 的后续修改取代，不可确认
...
48. 实时 Diff Preview 单文件查询接口（新增约定）

本节用于支持实时 Diff Preview 的“文件导航 + 选中文件查看对应 patch”交互。新增接口只读取

Coding 过程中已落库的 Preview revision，不代表正式 Diff，也不触发 commit、push、MR 或交付。

后端实现完成后生效；SSE 事件仍只负责通知 revision 更新，前端必须通过本节接口读取具体文件内容。
...
说明
GET
/api/v1/projects/{projectId}/tasks/{taskId}/workspace-diff-preview/file
Project Member + Task 需求群成员
查询指定 Preview revision 中单个文件的 patch
...
long
否
Preview revision；缺省取该 Task 的最新 revision
repositoryId
UUID
是
文件所属项目仓库 ID；多仓库场景必须传入，防止同名文件混淆
path
string
是
Workspace 内相对文件路径，必须进行 URL 编码；不得传宿主机绝对路径

repositoryId + path 唯一确定一个文件。path 不能包含 .. 越界片段，服务端不得信任客户端

提交的 workspaceId 或仓库路径，仓库归属必须从项目、Workspace 和 Preview revision 服务端数据解析。
...
- revision 为实际返回的 Preview revision；前端不能用请求中的 revision 覆盖该值。
- changeType 与正式 Diff 一致：ADDED / MODIFIED / DELETED / RENAMED。
- additions、deletions 为非负整数。
- binary=true 时 patch 必须为 null；二进制文件只返回元数据摘要，不返回源码内容。
- 文本文件的 patch 只包含当前文件对应的 unified diff，不返回其他文件 patch。
- Preview 快照已清理或读取失败时，仍返回文件元数据但 patch=null；前端应展示“该文件的实时
预览暂不可用”，不能将其当作正式 Diff 缺失。
  
  
  
48.3 错误与权限

- Task 不存在、不属于 projectId、没有 Workspace、revision 不存在，或指定文件不属于该
revision：返回 404 WORKSPACE_DIFF_PREVIEW_NOT_FOUND。
- 当前用户不是项目成员：返回项目统一的未授权错误。
- 当前用户是项目成员但不属于 Task 需求群：返回 403 GROUP_MEMBER_REQUIRED。
- repositoryId 缺失、格式错误、path 为空或路径越界：返回 400 INVALID_REQUEST。
- Preview 查询失败不应改变 Task、TaskRun、正式 Diff 或交付状态。
  
  
  
48.4 前端实时接入顺序

1. Coding 写入成功后，后端先落库 Preview revision，再发布 workspace.diff-preview.updated SSE 事件。
2. 前端收到事件后，以事件中的 revision 作为缓存失效标记，重新请求 /workspace-diff-preview/files。
3. 左侧文件列表使用 /files 返回的结构化条目，并按 repositoryPath 或仓库映射分组。
4. 用户选中文件后，请求本节 /file 接口，只展示该文件的 patch。
5. 用户切换文件或 revision 时取消/覆盖旧请求，避免旧 revision 的响应覆盖最新选择。
6. SSE 断线、事件乱序或续传失败时，以 REST 返回的最新 revision 为准；不得直接展示 SSE payload 中不存在的
patch，也不得刷新正式 diffs 接口代替 Preview 查询。
  
  
  
48.5 多仓库和兼容性要求

- 同一 Workspace 的不同仓库可以存在同名 path；前端必须始终携带 repositoryId。
- 旧客户端只使用 /workspace-diff-preview 和 /files 时仍保持兼容，但只能展示聚合 patch 或文件摘要，
不保证单文件精准预览。
- 本接口不改变现有 Preview 详情接口和 /files 接口的响应结构，不新增数据库字段；复用已有 revision、快照
和权限校验。
