...
重要说明：创建请求中的 repositoryIds 与 baseRef 属于随之创建的 Workspace（worktree），不属于 Task 本身；tasks 表不存这两列。前端创建 Task 时带上它们是告诉系统用哪些仓库、以哪个分支为基准初始化 Workspace。
...
项目成员
项目级 Diff 列表，支持 taskId 过滤与游标分页（limit 默认 20，上限 100）
GET
