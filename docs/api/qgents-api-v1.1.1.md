...
- 项目总群与需求群统一为 Group，使用 PROJECT\_MAIN 与 REQUIREMENT 区分。
...
|角色|作用域|主要权限|
|---|---|---|
|`TEAM_OWNER`|团队\<br\>|创建/归档项目；邀请、移除团队成员；管理团队级 GitHub App 集成；跨项目兜底管理|
...
|`GET`|`/teams/{teamId}/invitations`|Team Owner|查询邀请状态\<br\>|
...
受邀邮箱尚未注册时，系统发送注册链接和邀请令牌；用户以相同邮箱注册后可接受邀请。不可移除最后一名 Team Owner。

### 5\.2 项目与项目成员

|方法|路径|权限|说明|
|---|---|---|---|
|`POST`|`/teams/{teamId}/projects`|Team Owner\<br\>|创建项目|
...
|`PATCH`|`/projects/{projectId}/members/{userId}`|Project Admin\<br\>|在 `PROJECT_MEMBER`/`PROJECT_ADMIN` 间调整|
...
|`GET`|`/teams/{teamId}/integrations/github/installations`|Team Owner\<br\>|团队已安装的 GitHub App 列表|
...
|`POST`|`/projects/{projectId}/groups/{groupId}/archive`|创建者或 Project Admin<br>|归档需求群|
...
|`GET`<br>|`/projects/{projectId}/orchestration-runs`|项目成员|任务中心查询，支持 `groupId`、状态、发起人、游标过滤|
...
|`GET`/`POST`<br>|`/projects/{projectId}/diffs/{diffId}/comments`|Project Member|查询或添加 Diff 审查意见|
...
CQ 审查者不得是 MR 作者、交付物创建者或代表其执行的 Agent；服务端须记录审查者、时间、所审查的 `headCommit` 和理由。新提交推送到 MR 后，旧 CQ\\\+1 是否失效由目标分支门禁配置决定，并反映到 `qualityGate`。
...
> 本节由前端在 Mock 开发和联调过程中记录。
> 不代表后端接口已经实现，确认后再回填正式契约。

### FE-API-001 SSE 鉴权方式待确认
