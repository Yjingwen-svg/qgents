...
export const agentApi = {
  ...
  assignments(projectId: string, agentId: string, type: AgentAssignmentType, cursor?: string, limit?: number) {
    return requestPage<AgentAssignmentSummary>(withQuery(`/projects/${projectId}/agents/${agentId}/assignments`, { type, cursor, limit }))
  },
  taskRuns(projectId: string, filters: { agentId: string; status?: AgentTaskRunSummary['status']; cursor?: string; limit?: number }): Promise<CursorPage<AgentTaskRunSummary>> {
    return requestPage<AgentTaskRunSummary>(withQuery(`/projects/${projectId}/task-runs`, filters))
  },
}
