...
const projectAgent = (agent: AgentDetail): AgentDetail => agent
...
export const agentHandlers = [
  http.get('*/api/teams/:teamId/agents', ({ params, request }) => {
    ...
    const data = store(value(params, 'teamId')).filter((agent) => agent.createdBy === currentUserId).map((agent) => projectAgent(agent)).map(({ prompt: _prompt, ...agent }) => agent)
    return page(data, request)
  }),
  http.get('*/api/teams/:teamId/agents/:agentId', ({ params }) => { const agent = store(value(params, 'teamId')).find((item) => item.id === value(params, 'agentId') && item.createdBy === currentUserId); return agent ? response(projectAgent(agent)) : error(404, 'AGENT_NOT_FOUND') }),
