...
export const agentHandlers = [
  ...
  http.get('*/api/projects/:projectId/agent-skill-bindings/:agentId', ({ params }) => {
    ...
    const data: AgentSkillBindingResponse = { agentId: agent.id, skillIds: agent.id === 'agent-private-backend' ? ['skill-typescript', 'skill-api'] : [], skills: agent.id === 'agent-private-backend' ? [{ id: 'skill-typescript', name: 'TypeScript', visibility: 'PROJECT_SHARED', status: 'PUBLISHED' }, { id: 'skill-api', name: 'API 设计', visibility: 'TEAM_SHARED', status: 'PUBLISHED' }] : [], updatedAt: '2026-08-13T00:00:00Z' }
