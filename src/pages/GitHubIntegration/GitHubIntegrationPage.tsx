...
function GitHubIntegrationPageInner() {
  ...
  function repoCountOf(inst: GithubInstallation): number {
    if (typeof inst.authorizedRepoCount === "number")
      return inst.authorizedRepoCount;
  ...
  return (
    <>
      ...
      >
        <div>
          ...
          <Paragraph type="secondary" style={{ marginBottom: 0 }}>
            团队级 GitHub App
            授权与项目仓库绑定。代码操作由服务端受控执行，前端不持有 Git 凭据。
            {" · "}
      ...
      <Card
        ...
        extra={
          <Text type="secondary">
            当前团队已关联 {installations.length} 个安装
          </Text>
        }
        style={{ marginBottom: 16 }}
      >
      ...
      <Card title="说明" type="inner">
        ...
        >
          <li>
            GitHub App 授权为<strong>团队级</strong>；一次安装对应一个
            <strong>个人账号</strong>或<strong>组织</strong>（安装时在 GitHub
            上选择）。
          </li>
          <li>
            「查看仓库」进入独立页面；仓库后的「绑定该仓库到项目」会进入本团队项目列表（Owner
            可见全部项目）。
          </li>
          <li>
            点击「安装Github App」→ POST 获取 <code>installationUrl</code> →
            跳转 GitHub；回调由后端
